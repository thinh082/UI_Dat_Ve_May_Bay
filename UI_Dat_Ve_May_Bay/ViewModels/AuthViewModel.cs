...
namespace UI_Dat_Ve_May_Bay.ViewModels
{
    ...
    public class AuthViewModel : ObservableObject
    {
        ...
        private const string HubUrlHttps = "https://localhost:5231/hubs/notify";
        ...
        public AuthViewModel(AuthApi authApi, TokenStore tokenStore, Action onLoginSuccess)
            : this(
                  ...
                  new ApiClient("http://localhost:5231"),
                  onLoginSuccess)
        {
        }
        ...
        private async Task SocialLoginAsync(SocialProvider provider)
        {
            ...
            try
            {
                ...
                StatusMessage = "Đang mở trình duyệt để đăng nhập...";
            }
            catch (Exception ex)
            {
        ...
        private async Task SendOtpAsync()
        {
            ...
            try
            {
                var (ok, msg) = await _authApi.ForgotPasswordSendOtpAsync(FpEmail?.Trim() ?? "");
                StatusMessage = msg;
        ...
        private async Task VerifyOtpAsync()
        {
            ...
            try
            {
                var (ok, msg) = await _authApi.VerifyOtpAsync(FpEmail?.Trim() ?? "", FpOtp?.Trim() ?? "");
                StatusMessage = msg;
        ...
        private async Task ResetPasswordAsync()
        {
            ...
            try
            {
                var (ok, msg) = await _authApi.ResetPasswordAsync(
                    FpEmail?.Trim() ?? "",
                    FpNewPassword ?? "",
                    FpConfirmPassword ?? "");

                StatusMessage = msg;
                if (ok) ResetForgotFlow();
            }
            catch (Exception ex)
            {
